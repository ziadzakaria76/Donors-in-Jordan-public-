...
| **Verified offline against fixtures** | Extraction cascade, quality gate, parsers, filtering, scoring, deduplication, reporting, all output formats, alerting, `--capture`, scraper resilience, the portal list and its validation — **2,527 checks** |
...
```bash
python tests/run_all.py    # 2,527 checks, no network, no credentials
```
